class Product < ApplicationRecord
  ...
  validates :amount, numericality: { only_integer: true, message: "Amount must be an integer" }, if: proc { |product| !product.amount.blank? }
  validates :amount, presence: { message: "Amount can't be blank" }
  validates :msrp, presence: { message: "MSRP can't be blank" }
  validates :msrp, numericality: { message: "MSRP must be a number" }, if: proc { |product| !product.msrp.blank? }
  validates :price, presence: { message: "Price can't be blank" }
  validates :price, numericality: { greater_than_or_equal_to: 0, message: "Price must be greater than or equal to 0" }, if: proc { |product| !product.price.blank? }
